...
$pixelBase = [
    'meta'   => ['pixel_id'=>'','token'=>'','page_load_event'=>'','form_submit_event'=>'','thanks_page_event'=>'','value'=>'','currency'=>'IDR','test_event_code'=>''],
    'tiktok' => ['pixel_id'=>'','access_token'=>'','page_load_event'=>'','form_submit_event'=>'','thanks_page_event'=>'','value'=>'','currency'=>'IDR','test_event_code'=>''],
    'google' => ['gtm_id'=>'','conversion_id'=>'','ga4_id'=>'','page_load_label'=>'','form_submit_label'=>'','thanks_page_label'=>'','value'=>'','currency'=>'IDR'],
    'snack'  => ['pixel_id'=>'','access_token'=>'','page_load_event'=>'','form_submit_event'=>'','thanks_page_event'=>'','value'=>'','currency'=>'IDR','test_mode'=>'','test_click_id'=>'','html'=>''],
];
...
<div class="page-wrapper" id="campApp">
  ...
  <!-- Tabs nav -->
  <div class="tabs-list" id="tabNav">
    <button class="tab-trigger active" onclick="KNK.tab('basic',this)">Informasi Dasar</button>
    <button class="tab-trigger" id="tabnav_form" onclick="KNK.tab('form',this)">Form &amp; Tampilan</button>
    <button class="tab-trigger"        onclick="KNK.tab('thanks',this)">Halaman Thanks</button>
    <button class="tab-trigger"        onclick="KNK.tab('pixel',this)">Pixel &amp; Tracking</button>
    <button class="tab-trigger"        onclick="KNK.tab('ops',this)">Operator CS</button>
  </div>
  ...
  <div id="tab-thanks" class="tab-pane" style="display:none;flex-direction:column;gap:16px;">
    ...
    <div class="card">
      <div class="card-header"><h3 class="card-title">Kustomisasi Tema</h3></div>
      <div class="card-content">
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;">
          <div>
            <label class="label" style="font-size:11px;">Icon / Karakter</label>
            <input id="th_icon" class="input" style="font-size:13px;" placeholder="Masukkan karakter"
              oninput="KNK.setTheme('icon',this.value)">
          </div>
          ...
          <?php foreach([['Warna Background','bg_color','#f0fdf4'],['Warna Card','card_color','#ffffff'],['Warna Aksen','accent_color','#16a34a'],['Warna Teks','text_color','#0f172a']] as [$lbl,$k,$ph]): ?>
          <div>
            <label class="label" style="font-size:11px;"><?= ae($lbl) ?></label>
            <div style="display:flex;gap:6px;">
              ...
              <input type="text" id="th_<?= $k ?>" class="input" style="font-size:12px;padding:6px 10px;" placeholder="<?= ae($ph) ?>"
  ...
  <div id="tab-pixel" class="tab-pane" style="display:none;flex-direction:column;gap:12px;">
    ...
    <!-- ── META ──────────────────────────────────────────────────── -->
    <details class="card card-details" open>
      <summary style="display:flex;align-items:center;gap:10px;">
        <span style="width:10px;height:10px;border-radius:50%;background:#1877f2;flex-shrink:0;"></span>
        Meta / Facebook CAPI v21.0
      </summary>
      <div class="details-body" style="display:flex;flex-direction:column;gap:14px;">

        <!-- Kredensial -->
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;">
          <div>
            <?= pxLabel('Pixel ID') ?>
            <input id="px_meta_pixel_id" class="input" style="font-size:12px;padding:6px 10px;" placeholder="ID1,ID2,ID3 (pisah koma)"
    ...
    <details class="card card-details">
      <summary style="display:flex;align-items:center;gap:10px;">
        <span style="width:10px;height:10px;border-radius:50%;background:#010101;flex-shrink:0;"></span>
    ...
    <details class="card card-details">
      <summary style="display:flex;align-items:center;gap:10px;">
        <span style="width:10px;height:10px;border-radius:50%;background:#e4372c;flex-shrink:0;"></span>
        SnackVideo / Kwai Ads
      </summary>
      <div class="details-body" style="display:flex;flex-direction:column;gap:14px;">
    ...
    <details class="card card-details">
      <summary style="display:flex;align-items:center;gap:10px;">
        <span style="width:10px;height:10px;border-radius:50%;background:#4285f4;flex-shrink:0;"></span>
        Google Ads / GTM / GA4
      </summary>
      <div class="details-body" style="display:flex;flex-direction:column;gap:14px;">
...
const KNK = (() => {
  ...
  function updateCampTypeBadge(type) {
    ...
    // Form & Tampilan tab — hide for wa_link
    var formTab = $('tabnav_form');
    if(formTab) formTab.style.display = isWa ? 'none' : '';
    // page desc
    var desc = $('page_desc_main');
    if(desc) desc.textContent = isWa
      ? 'Atur kampanye WA Link, halaman thanks, tracking pixel, dan routing CS'
    ...
    if(prevTabLabel) prevTabLabel.textContent = isWa ? 'WA Link' : 'Form';
    // pixel step 2 labels
  ...
  function buildFormPreview() {
    ...
    if (pxCfg.meta && pxCfg.meta.pixel_id) pxBadges.push('<span style="display:inline-flex;align-items:center;gap:4px;font-size:10px;font-weight:600;background:#1877f2;color:#fff;padding:3px 8px;border-radius:99px;"><span style="width:6px;height:6px;border-radius:50%;background:#fff;"></span>Meta</span>');
    if (pxCfg.tiktok && pxCfg.tiktok.pixel_id) pxBadges.push('<span style="display:inline-flex;align-items:center;gap:4px;font-size:10px;font-weight:600;background:#010101;color:#fff;padding:3px 8px;border-radius:99px;"><span style="width:6px;height:6px;border-radius:50%;background:#fff;"></span>TikTok</span>');
    if (pxCfg.google && (pxCfg.google.conversion_id || pxCfg.google.gtm_id || pxCfg.google.ga4_id)) pxBadges.push('<span style="display:inline-flex;align-items:center;gap:4px;font-size:10px;font-weight:600;background:#4285f4;color:#fff;padding:3px 8px;border-radius:99px;"><span style="width:6px;height:6px;border-radius:50%;background:#fff;"></span>Google</span>');
    if (pxCfg.snack && pxCfg.snack.pixel_id) pxBadges.push('<span style="display:inline-flex;align-items:center;gap:4px;font-size:10px;font-weight:600;background:#e4372c;color:#fff;padding:3px 8px;border-radius:99px;"><span style="width:6px;height:6px;border-radius:50%;background:#fff;"></span>Snack</span>');
  ...
  function buildWaLinkPreview() {
    ...
    if (pxCfg.snack && pxCfg.snack.pixel_id) pxBadges.push('<span style="display:inline-flex;align-items:center;gap:4px;font-size:10px;font-weight:600;background:#e4372c;color:#fff;padding:3px 8px;border-radius:99px;"><span style="width:6px;height:6px;border-radius:50%;background:#fff;"></span>Snack</span>');
    var pxBar = pxBadges.length ? '<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:10px;">' + pxBadges.join('') + '</div>' : '';
    return '<div style="background:'+bg+';padding:28px 20px;border-radius:8px;font-family:-apple-system,BlinkMacSystemFont,sans-serif;">'+
      pxBar +
      '<div style="max-width:380px;margin:0 auto;">'+header+
      '<div style="display:flex;align-items:center;justify-content:center;gap:10px;width:100%;padding:15px 20px;background:'+acc+';color:'+btnC+';border-radius:10px;font-size:'+bfz+';font-weight:700;cursor:default;">'+
      waIcon+' '+label+
      '</div>'+
      '<div style="text-align:center;margin-top:12px;font-size:11px;color:'+txt+';opacity:.5;">Klik tombol untuk terhubung ke CS via WhatsApp</div>'+
      '</div></div>';
  }

  function buildThanksPreview() {
    const th=d.thanksCfg.theme||{};
    const bg=th.bg_color||'#f0fdf4', card=th.card_color||'#fff', acc=th.accent_color||'#16a34a', txt=th.text_color||'#0f172a';
  ...
  function save() {
    ...
    sv('f_thanks_page_config_json', JSON.stringify(d.thanksCfg));
  ...
  function populateUI() {
    ...
    const th=d.thanksCfg.theme||{};
    ['icon','heading','bg_color','card_color','accent_color','text_color'].forEach(k=>{
      sv('th_'+k, th[k]||'');
      const cel=$('th_c_'+k); if(cel&&th[k]) cel.value=th[k];
    });
  ...
  return { d, tab, onNameInput, toggle, onRedirectType, setFormTpl, setSize, setThanksTpl,
           setStyle, setTheme, setPixel, addExtraField, removeExtraField,
