...
function buildSnippetData($c)
{
    ...
    // ── Scheme ───────────────────────────────────────────────────────────
    $schemes = [
        'modern'  =>['#ffffff','#2563eb','#0f172a','#e2e8f0','#ffffff','#f8fafc'],
        'classic' =>['#ffffff','#dc2626','#1a1a1a','#d4a27a','#ffffff','#ffffff'],
        'minimal' =>['#fafafa','#111111','#111111','#e2e8f0','#ffffff','#ffffff'],
        'card'    =>['#eff6ff','#2563eb','#0f172a','#bfdbfe','#ffffff','#ffffff'],
        'gradient'=>['#1e3a5f','#38bdf8','#e0f2fe','rgba(255,255,255,.2)','#0f172a','rgba(255,255,255,.1)'],
        'rose'    =>['#fff1f2','#e11d48','#0f172a','#fecdd3','#ffffff','#ffffff'],
        'forest'  =>['#f0fdf4','#16a34a','#0f172a','#bbf7d0','#ffffff','#ffffff'],
        'sunset'  =>['#fff3e0','#ee5a24','#2d3436','rgba(238,90,36,.3)','#ffffff','#ffffff'],
        'ocean'   =>['#e0f7fa','#0077b6','#003049','#90e0ef','#ffffff','#ffffff'],
    ];
    $sizes = [
        'compact'  =>['380px','20px 18px','13px','14px'],
        'default'  =>['480px','28px 24px','14px','15px'],
        'large'    =>['600px','40px 36px','15px','16px'],
        'fullwidth'=>['100%', '28px 24px','14px','15px'],
    ];
    $tpl = isset($formCfg['template']) ? $formCfg['template'] : 'modern';
    $sz  = isset($formCfg['size'])     ? $formCfg['size']     : 'default';
    list($bg,$acc,$txt,$bdr,$btnTxt,$inpBg) = isset($schemes[$tpl]) ? $schemes[$tpl] : $schemes['modern'];
    list($maxW,$pad,$fsize,$bfsize)          = isset($sizes[$sz])   ? $sizes[$sz]   : $sizes['default'];

    $bg     = !empty($cs['color_bg'])       ? $cs['color_bg']       : $bg;
    $acc    = !empty($cs['color_accent'])   ? $cs['color_accent']   : $acc;
    $txt    = !empty($cs['color_text'])     ? $cs['color_text']     : $txt;
    $bdr    = !empty($cs['color_border'])   ? $cs['color_border']   : $bdr;
    $btnTxt = !empty($cs['color_btn_text']) ? $cs['color_btn_text'] : $btnTxt;
    $inpBg  = !empty($cs['input_bg'])       ? $cs['input_bg']       : $inpBg;
    $maxW   = !empty($cs['max_width'])      ? $cs['max_width']      : $maxW;
    $pad    = !empty($cs['padding'])        ? $cs['padding']        : $pad;
    $fsize  = !empty($cs['font_size'])      ? $cs['font_size']      : $fsize;
    $bfsize = !empty($cs['btn_font_size'])  ? $cs['btn_font_size']  : $bfsize;
    ...
    $submitUrlJs = addslashes($submitUrl);

    // ── Form embed code ───────────────────────────────────────────────────
    $cardBg = ($tpl === 'gradient') ? 'rgba(255,255,255,.08)' : '#ffffff';
    $formCode = <<<HTML
<!-- Konektor Form: {$c->name} -->
{$metaSc}{$tiktokSc}{$googleSc}
<style>
.knk-wrap{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:{$bg};min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px;}
.knk-card{width:100%;max-width:{$maxW};background:{$cardBg};border-radius:{$radius};padding:{$pad};box-shadow:0 4px 24px rgba(0,0,0,.08);}
...
<div class="knk-wrap">
  <div class="knk-card">
    ...
    <form id="knkForm" novalidate>
      {$fh}
      <button type="submit" id="knkBtn" disabled style="width:100%;padding:13px;font-size:{$bfsize};font-weight:700;background:{$acc};color:{$btnTxt};border:none;border-radius:8px;cursor:pointer;font-family:inherit;">{$btnLbl}</button>
    </form>
  </div>
</div>
<script>
...
HTML;
    ...
    $waIcon    = '<svg style="width:20px;height:20px;fill:currentColor;flex-shrink:0" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.123.554 4.117 1.528 5.852L0 24l6.335-1.508A11.945 11.945 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.818a9.818 9.818 0 01-5.002-1.368l-.36-.214-3.72.885.916-3.617-.234-.372A9.818 9.818 0 012.182 12C2.182 6.57 6.57 2.182 12 2.182S21.818 6.57 21.818 12 17.43 21.818 12 21.818z"/></svg>';

    if ($c->type === 'wa_link') {
        // Link: simple anchor — server handles redirect to CS + lead tracking
        $btnCode = <<<HTML
<!-- Konektor Link Button: {$c->name} -->
<style>
.knk-btn-wrap{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;text-align:center;}
.knk-link-btn{display:inline-flex;align-items:center;justify-content:center;gap:10px;width:100%;max-width:360px;padding:15px 20px;background:{$acc};color:{$btnTxt};border:none;border-radius:10px;font-size:{$bfsize};font-weight:700;cursor:pointer;text-decoration:none;font-family:inherit;transition:opacity .2s;}
.knk-link-btn:hover{opacity:.88;}
</style>
<div class="knk-btn-wrap">
  <a href="{$url}" class="knk-link-btn">
    {$waIcon} {$linkLabel}
  </a>
</div>
...
HTML;
        ...
        // Form: button that navigates to the form page
        $btnCode = <<<HTML
<!-- Konektor Button: {$c->name} -->
{$metaSc}{$tiktokSc}{$googleSc}
<style>
.knk-btn-wrap{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;text-align:center;}
.knk-link-btn{display:inline-flex;align-items:center;justify-content:center;gap:10px;width:100%;max-width:360px;padding:15px 20px;background:{$acc};color:{$btnTxt};border:none;border-radius:10px;font-size:{$bfsize};font-weight:700;cursor:pointer;text-decoration:none;font-family:inherit;transition:opacity .2s;}
  ...
  <div class="dialog-content" style="max-width:720px;width:100%;">
    ...
    <div style="padding:12px 24px 0;">
      <div style="display:flex;gap:4px;border-bottom:1px solid hsl(var(--border));margin-bottom:16px;">
        <button id="stab-form" onclick="switchSnippetTab('form')"
          ...
          <?= icon('file-text','',13) ?> Kode Form
        </button>
        <button id="stab-btn" onclick="switchSnippetTab('btn')"
          ...
          <?= icon('code','',13) ?> <span id="stab-btn-label">Kode HTML</span>
        </button>
    ...
    <!-- Tab: Form Embed -->
    <div id="spane-form" class="snippet-pane" style="padding:0 24px 20px;">
      <p style="font-size:12px;color:hsl(var(--muted-foreground));margin-bottom:10px;">
        Tempel kode <code style="background:hsl(var(--muted));padding:1px 5px;border-radius:3px;">&lt;form&gt;</code> ini ke halaman Anda.
        Pixel tracking Meta, TikTok, Google sudah terintegrasi. Alur event: <strong>Form Load &rarr; Form Submit &rarr; Thanks Page</strong>.
      </p>
      <div style="position:relative;">
        <textarea id="code-form" readonly
          style="width:100%;height:220px;font-family:'JetBrains Mono','Cascadia Code','Fira Code',monospace;font-size:11px;line-height:1.6;
        ...
        <button onclick="copySnippet('form')" id="copy-form"
          style="position:absolute;top:8px;right:8px;" class="btn btn-secondary btn-sm">
          <?= icon('copy') ?> Salin
        </button>
      </div>
      <div style="margin-top:10px;padding:10px 12px;background:hsl(217 91% 60%/0.08);border:1px solid hsl(217 91% 60%/0.2);border-radius:var(--radius);font-size:12px;color:hsl(217 91% 40%);">
        <?= icon('info','',13) ?> Paste kode ini di dalam <code>&lt;body&gt;</code>. Untuk embed parsial, wrap dengan container dan ubah <code>.knk-wrap{min-height:auto}</code>.
      </div>
    </div>

    <!-- Tab: Tombol / Button -->
    <div id="spane-btn" class="snippet-pane" style="display:none;padding:0 24px 20px;">
      <p id="spane-btn-desc" style="font-size:12px;color:hsl(var(--muted-foreground));margin-bottom:10px;">
        Kode tombol yang bisa dipasang di mana saja — WordPress, landing page, blog. Pixel + tracking sudah terintegrasi.
      </p>
      <div style="position:relative;">
        <textarea id="code-btn" readonly
          style="width:100%;height:200px;font-family:'JetBrains Mono','Cascadia Code','Fira Code',monospace;font-size:11px;line-height:1.6;
                 border:1px solid hsl(var(--border));border-radius:var(--radius);padding:12px 14px;resize:none;
                 background:hsl(222.2 84% 4.9%);color:hsl(142 76% 55%);outline:none;"></textarea>
        <button onclick="copySnippet('btn')" id="copy-btn"
          style="position:absolute;top:8px;right:8px;" class="btn btn-secondary btn-sm">
          <?= icon('copy') ?> Salin
        </button>
      </div>
...
function openSnippet(id) {
  ...
  document.getElementById('code-form').value  = s.form_code || '';
  document.getElementById('code-btn').value   = s.btn_code  || '';
  document.getElementById('url-direct').value = s.url;
  document.getElementById('url-open').href    = s.url;

  // Tab label: always "Kode HTML" for both types
  var btnLabelEl = document.getElementById('stab-btn-label');
  if (btnLabelEl) btnLabelEl.textContent = 'Kode HTML';

  // Pane description
  var btnDescEl = document.getElementById('spane-btn-desc');
  if (btnDescEl) btnDescEl.textContent = isWa
    ? 'Kode tombol HTML yang bisa dipasang di website, WordPress, atau landing page Anda. Klik tombol langsung ke CS.'
    : 'Kode HTML form yang bisa dipasang di website, WordPress, atau landing page Anda.';

  // Form tab: hide for wa_link — link kampanye langsung dieksekusi, tidak perlu embed form
  var formTab = document.getElementById('stab-form');
  formTab.style.display = isWa ? 'none' : '';

  // Reset demo frame — use /demo/ URL (no tracking)
  ...
  // URL pane description — for Link: explain it's a direct action link
  ...
  switchSnippetTab(isWa ? 'url' : 'form');
...
function switchSnippetTab(tab) {
  ...
  ['form','btn','url','demo'].forEach(function(t) {
  ...
  // Load demo iframe on demand — use /demo/ prefix URL
