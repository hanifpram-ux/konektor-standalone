...
$daily   = Analytics::getDailyLeads($campId,$days);
$stats   = $campId ? Analytics::getCampaignStats($campId,$days) : Analytics::getOverallStats();
...
<div class="page-wrapper">
  <div class="page-header">
    <div>
      ...
      <p class="page-desc">Performa kampanye dan lead dalam <?= $days ?> hari terakhir</p>
    </div>
    <form method="GET" style="display:flex;gap:8px;">
      ...
      <select name="days" class="input" style="width:120px;">
        <?php foreach ([7,14,30,60,90] as $d): ?><option value="<?= $d ?>" <?= $days===$d?'selected':'' ?>><?= $d ?> Hari</option><?php endforeach; ?>
      </select>
  ...
  <div style="display:grid;grid-template-columns:3fr 2fr;gap:20px;margin-bottom:20px;">
    <div class="card">
      <div class="card-header"><h3 class="card-title">Lead per Hari</h3><p class="card-desc"><?= $days ?> hari terakhir</p></div>
      <div class="card-content"><canvas id="barChart" height="90"></canvas></div>
    </div>
    <div class="card">
      <div class="card-header"><h3 class="card-title">Tren Lead</h3><p class="card-desc">Grafik garis</p></div>
